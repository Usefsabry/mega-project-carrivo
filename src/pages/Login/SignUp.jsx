  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await registerApi({
        fullName: formData.name,      // عدّلي الأسماء حسب الباك إند لو مختلفة
        email: formData.email,
        password: formData.password,
        role: "Student",
      });

      // لو الباك إند بيرجع توكن بعد التسجيل:
      if (data?.token) {
      ...
      alert(err.message || "Sign up failed. Please try again later.");
    }
  };

  const handleSocialLogin = (provider) => {
    console.log(`Sign up with ${provider}`);
    // Social sign-up logic هنا لو هـتستخدميه
  };
                  ...
                  >
                    {showPassword ? (
                      ...
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        ...
                        ircle cx="12" cy="12" r="3" strokeWidth="2"/
                      </svg>
                    )}
                  </button>
                </div>
